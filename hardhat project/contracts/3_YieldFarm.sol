...
contract LiquidityExamples is IERC721Receiver {
    ...
    function mintNewPosition(address tokenA, address tokenB, uint256 amountA, uint256 amountB)
        external
        returns (
            uint256 tokenId,
            uint128 liquidity,
        ...
        )
    {
        ...
        uint256 amount0ToMint = amountA;
        uint256 amount1ToMint = amountB;
    ...
    function decreaseLiquidity(uint256 tokenId, uint128 lAmount) external returns (uint256 amount0, uint256 amount1) {
        // caller must be the owner of the NFT
        require(msg.sender == deposits[tokenId].owner, 'Not the owner');
        // get liquidity data for tokenId
        uint128 liquidity = deposits[tokenId].liquidity;
        if( liquidity < lAmount){
            lAmount = liquidity;
        }
